...
valid_pred = best_model.predict(X_valid).round(0)

print('\nПервые предсказания для test:')
print(np.round(valid_pred[:10], 6))
...
plt.scatter(y_valid, valid_pred, alpha=0.4, color='steelblue')
...
plt.scatter(valid_pred, residuals, alpha=0.35)
